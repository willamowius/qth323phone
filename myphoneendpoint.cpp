...
void CMyPhoneEndPoint::OnConnectionCleared(H323Connection & connection, const PString & clearedCallToken)
{
	...
	{
		QString msg = QString("” %1 зан€то...").arg(m_dialog->FindContactName(connection));
...
{
	OnLogicalChannel(channel, "ќтправл€ем %1 данные %2.", "ѕолучаем %1 данные %2.");
	return TRUE;
}

void CMyPhoneEndPoint::OnClosedLogicalChannel(H323Connection &, const H323Channel &channel)
{
	OnLogicalChannel(channel, "ѕрекратили отправл€ть %1 данные %2.", "ѕрекратили получать %1 данные %2.");
}
